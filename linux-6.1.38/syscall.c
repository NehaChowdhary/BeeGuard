...
void print_struct_hex(const void *ptr, size_t size, const char *msg) {
    const unsigned char *byte_ptr = (const unsigned char *)ptr;
    size_t i;
    if(msg){
    	printk(KERN_INFO "%s :size [%zu] in hex: ",msg,size);
    }else{
    	printk(KERN_INFO "Print Struct of size [%zu] in hex: ",	size);
    }
    for (i = 0; i < size; i++) {
...
static int bpf_prog_load(union bpf_attr *attr, bpfptr_t uattr)
{
	...
	char license[128];
	bool is_gpl;
	const char* msg="Object: struct bpf_prog *prog";
	...
	/*******************************/
	/// Print Struct in Hex
	print_struct_hex(prog, sizeof(*prog),msg);
    	/*******************************/
